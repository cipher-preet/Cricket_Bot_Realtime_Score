...
def verifyandloginuser(logindetails: LoginModel, response: Response ):
    try:
        userauth = Userdetails()
        userData = userauth.verify_and_login_user(logindetails, response)
        
        print("userData", userData)
       
        # return JSONResponse(
        #     status_code=200,
        #     content=json_util.loads(json_util.dumps({
        #         "success": True,
        ...
        # )     
        return JSONResponse(
            status_code=200, content={"success": True,  "message":userData["message"], "user_data": userData["user_id"]}
        )
    except Exception as e:
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content= {"success": False, "message": str(e)})
...
@router.get("/verifyuserdata", summary="Verify user cookie", description="Api to verify user cookie")
def verify_user_cookie_data(request: Request):
    try:
        userauth = Userdetails()
        user_data = userauth.verify_user_cookie(request)
        if not user_data:
            return JSONResponse(
                status_code=401, content={"success": False, "message": "User not authenticated"}
            )
        return JSONResponse(
            status_code=200, content={"success": True, "user_data": user_data}
        )
    except Exception as e:
        print(traceback.format_exc())
        return JSONResponse(status_code=500, content={"success": False, "message": str(e)})